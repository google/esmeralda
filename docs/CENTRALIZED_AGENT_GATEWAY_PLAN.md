...
* **DNS Peering Configuration (`dnsPeeringConfig`)**:
  ...
  * *Constraint 1*: Must specify exact domain suffixes with trailing dot (`"esmeralda.internal."`). **Never use root wildcard (`.`) or `googleapis.com.`** to prevent rerouting public Google API traffic. *(Forensic Lesson from Phase 4: Including `"googleapis.com."` in `dns_peering_config` hijacked public Google APIs into Private DNS, causing A2A tool lookups and token minting to hang with resolution timeouts!)*
...
##### A. Required 8 Google APIs Endpoints (Tier 1 Baseline via `google_agent_registry_service`)
All agent runtimes require access to a standard set of Google Cloud APIs to discover tools, mint tokens, stream telemetry, and perform inference. Register these 8 endpoints declaratively inside `infrastructure/modules/5-governance/agent-gateway/main.tf` using the official Terraform resource salvaged from Phase 4:
...
> **CI/CD EXECUTION BOUNDARY**: Because the server resource ID (`registryResource`) is dynamically generated by Google Cloud upon registration, binding `roles/iap.egressor` inside `cloudbuild.yaml` immediately following `gcloud agent-registry services create` guarantees atomic registration and access authorization without causing 404 race conditions in Terraform.
...
##### Step-by-Step IAP Authorization for MCP Tools (`cloudbuild.yaml` CI/CD Step)
Immediately following `gcloud agent-registry services create` inside `cloudbuild.yaml`, retrieve the server-generated `registryResource` ID and bind `roles/iap.egressor` to the calling spoke project's `principalSet://` identity:
...
# 2. Grant roles/iap.egressor to A2A Spoke Project's principalSet identity with mandatory CEL condition
CALLING_PRINCIPAL="principalSet://agents.global.org-${_ORG_ID}.system.id.goog/attribute.platformContainer/aiplatform/projects/${_A2A_PROJECT_NUMBER}"
...
```bash
CALLING_PRINCIPAL="principalSet://agents.global.org-${_ORG_ID}.system.id.goog/attribute.platformContainer/aiplatform/projects/${_ROOT_PROJECT_NUMBER}"
...
#### 3.4 Central Model Armor Inspection & HTTP 799 Rejections (Reusing Phase 2 Templates)
* **Zero New Template Creation Required**: In Phase 2 (`feat/phase-2-model-armor`), we already provisioned two complete Model Armor guardrail templates inside the Central Governance Project (`var.governance_project_id`) via [`infrastructure/modules/5-governance/modules/5_model_armor/main.tf`](file:///usr/local/google/home/afonsomenegola/codigos/esmeralda/infrastructure/modules/5-governance/modules/5_model_armor/main.tf):
  * **Inbound Prompt Guardrails (`esmeralda-prompt-guardrails-dev`)**: Blocks prompt injection/jailbreaks (`MEDIUM_AND_ABOVE`), hate speech, harassment, sexually explicit content, and dangerous content.
  * **Outbound Response Guardrails (`esmeralda-response-guardrails-dev`)**: Enforces Sensitive Data Protection (DLP PII masking to `[EMAIL_ADDRESS]`, `[SSN]`, `[CREDIT_CARD_NUMBER]`) on responses returned from MCP servers back to agents.
* **Gateway Attachment**: When provisioning `google_network_services_agent_gateway` in Layer 5, we directly pass `module.model_armor.prompt_template_name` / `response_template_name` without re-creating templates.
...
#### 4.1 Cross-Project IAM Binding (Declarative Terraform in Layer 5 Governance)
Manage the custom least-privilege role and cross-project Vertex AI service agent bindings declaratively inside the Governance Project (`infrastructure/modules/5-governance/agent-gateway/main.tf`):

```hcl
# 1. Custom Least-Privilege Role in Central Governance Project
resource "google_project_iam_custom_role" "ae_agw_cross_project_sa" {
  project     = var.governance_project_id
  role_id     = "ae_agw_cross_project_sa"
...
# 2a. Allow Root Agent Vertex AI Service Agent to access Central Agent Gateway
resource "google_project_iam_member" "root_ae_agw_access" {
  project = var.governance_project_id
  role    = google_project_iam_custom_role.ae_agw_cross_project_sa.name
  member  = "serviceAccount:service-${var.root_project_number}@gcp-sa-aiplatform.iam.gserviceaccount.com"
}

# 2b. Allow A2A Agent Vertex AI Service Agent to access Central Agent Gateway
resource "google_project_iam_member" "a2a_ae_agw_access" {
  project = var.governance_project_id
  role    = google_project_iam_custom_role.ae_agw_cross_project_sa.name
  member  = "serviceAccount:service-${var.a2a_project_number}@gcp-sa-aiplatform.iam.gserviceaccount.com"
}
```

#### 4.2 Agent Manifest & Reasoning Engine Deployment Configuration (`agent_gateway_config`)

##### A. Developer Manifest (`agent.yaml`)
To ensure full transparency for Esmeralda developers, explicitly declare the complete suite of gateway proxy and SPIFFE identity environment variables inside the `env:` block of `apps/agents/a2a-agent/agent.yaml` and `base-adk-agent/agent.yaml`:

```yaml
# In apps/agents/a2a-agent/agent.yaml
name: 'a2a-mortgage-agent'
env:
  # 1. Disable legacy CAA token sharing opt-out so SPIFFE Agent Identity tokens authenticate across GCP APIs
  GOOGLE_API_PREVENT_AGENT_TOKEN_SHARING_FOR_GCP_SERVICES: 'False'
  # 2. Trigger entrypoint.sh to resolve Gateway Root CA cert from Secret Manager
  SECRET_NAME: 'agw-root-ca-cert'
  # 3. Prevent gRPC SDK IPv6 blackhole timeouts inside private containers
  GRPC_IPV6_PREFER: '0'
  # 4. Route GCP APIs through standard TLS over Agent Gateway instead of client-cert mTLS handshakes
  GOOGLE_API_USE_CLIENT_CERTIFICATE: 'false'
  GOOGLE_API_USE_MTLS_ENDPOINT: 'never'
  # 5. Route Gemini / ADK calls through Vertex AI platform endpoint
  GOOGLE_GENAI_USE_VERTEXAI: 'True'
```

##### B. Terraform Deployment Spec & REST `PATCH` Binding
In `infrastructure/modules/4-workloads/agents/a2a-agent/main.tf` and `base-adk-agent/main.tf`, set `identity_type = "AGENT_IDENTITY"`, remove `psc_interface_config`, and wire `agent_gateway_config`:
  ...
  deployment_spec {
    agent_gateway_config {
      agent_to_anywhere_config {
        agent_gateway = "projects/${var.governance_project_id}/locations/${var.region}/agentGateways/${var.central_gateway_name}"
      }
    }
  }
}
```
*Updating Existing Reasoning Engines*: For already-deployed `ReasoningEngine` instances, execute a Vertex AI REST API `PATCH` request with `updateMask=spec.deploymentSpec.agentGatewayConfig` to bind the central gateway without downtime:

```bash
curl -s -X PATCH \
  -H "Authorization: Bearer $(gcloud auth print-access-token)" \
  -H "Content-Type: application/json; charset=utf-8" \
  -d '{"spec":{"deploymentSpec":{"agentGatewayConfig":{"agentToAnywhereConfig":{"agentGateway":"projects/'"${_GOVERNANCE_PROJECT_ID}"'/locations/'"${_REGION}"'/agentGateways/esmeralda-central-gateway"}}}}}' \
  "https://${_REGION}-aiplatform.googleapis.com/v1beta1/${_REASONING_ENGINE_ID}?updateMask=spec.deploymentSpec.agentGatewayConfig"
```
...
1. **Dynamic CA Certificate Injection (`scripts/entrypoint.sh`, `pyproject.toml`, & `Dockerfile`)**:
   * **`pyproject.toml` Dependency**: Add `"google-cloud-secret-manager"` to `dependencies` so `entrypoint.sh` can query Secret Manager.
   * **`entrypoint.sh`**: At boot, fetch `agw-root-ca-cert` from Secret Manager and run `update-ca-certificates`:
     ```bash
     ...
     ```
   * **Mandatory `Dockerfile` Trust Paths**: Because `gRPC` (used by Vertex AI SDK) and `requests` (`certifi`) ignore Debian's `/etc/ssl/certs/ca-certificates.crt` by default, explicitly export system root trust paths in `Dockerfile`:
     ```dockerfile
...
1. **`infrastructure/modules/5-governance/agent-gateway/main.tf`**
   * Creates `google_compute_network_attachment` inside `var.governance_project_id` referencing `/28` subnet in `var.net_host_project_id`.
   * Creates `google_network_services_agent_gateway` (`AGENT_TO_ANYWHERE`) with `dnsPeeringConfig` targeting `esmeralda.internal.`.
   * Creates `google_agent_registry_service` (`system_endpoints`) declaratively registering the 8 mandatory Google API endpoints (`agentregistry`, `aiplatform.mtls`, etc.).
   * Creates `google_secret_manager_secret` (`agw_ca_cert`) publishing `google_network_services_agent_gateway.central_gateway.agent_gateway_card[0].root_certificates` so `entrypoint.sh` can inject the TLS inspection proxy root CAs.
   * Creates `google_model_armor_template` for prompt injection (`LOW_AND_ABOVE`) and PII masking (`[EMAIL_ADDRESS]`, `[SSN]`).
   * Creates custom role `ae_agw_cross_project_sa` and binds `root-agent` & `a2a-agent` Vertex AI service agents (`@gcp-sa-aiplatform-re...` and `@gcp-sa-aiplatform...`).
2. **`scripts/register_central_catalog.sh`**
   * Idempotently registers all 8 Google APIs endpoints into `var.governance_project_id`.
   * Registers `legacy-dms`, `income-verification`, `corporate-email`, and `a2a-mortgage-agent` into `var.governance_project_id`.
...
2. **`infrastructure/modules/2-networking/main.tf`**
   * Provision Cloud Router & Cloud NAT (`google_compute_router.nat_router`, `google_compute_router_nat.nat_gateway`) for internet-destined egress from the Shared VPC.
   * Provision dedicated `/24` egress subnet (`google_compute_subnetwork.agw_egress`) with `private_ip_google_access = true`.
   * Remove spoke agent Vertex AI service agents (`a2a_vertex_service_agent`, `root_vertex_service_agent`, and `-re` counterparts) from `local.subnet_network_users` on the Shared VPC host project.
   * Add the Central Agent Gateway Service Agent (`service-${var.governance_project_number}@gcp-sa-agentgateway.iam.gserviceaccount.com`) to `agw_egress` IAM users so it can attach to the egress subnet and peer DNS.
3. **`infrastructure/modules/3-security/main.tf` & `agent_identity.tf`**
   ...
   * In `agent_identity.tf`: Grant `roles/secretmanager.secretAccessor`, `roles/aiplatform.agentDefaultAccess`, and `roles/agentregistry.viewer` to `root_agent_identity_roles` and `a2a_agent_identity_roles`.
4. **`apps/services/legacy-dms/cloudbuild.yaml`, `income-verification/cloudbuild.yaml`, `corporate-email/cloudbuild.yaml`, `apps/agents/a2a-agent/cloudbuild.yaml` & MCP `terragrunt.hcl`**
   * In `cloudbuild.yaml`: Drop `alpha` flag from `gcloud agent-registry services create`, replace `gcloud projects list --filter="..."` loop with Two-Vault secret resolution (`secret-esmeralda-governance-id`), register service into `${GOV_PROJECT}`, and bind `roles/iap.egressor` to the calling spoke project's `principalSet://`.
   * In `terragrunt.hcl`: Update `invoker_service_accounts` to grant `roles/run.invoker` to the Agent Identity service agents (`service-${PROJECT_NUMBER}@gcp-sa-aiplatform.iam.gserviceaccount.com`) and Central Agent Gateway Service Agent as legacy service accounts are phased out.
5. **`infrastructure/modules/4-workloads/agents/a2a-agent/main.tf`, `base-adk-agent/main.tf` & `agent.yaml`**
   * In `main.tf`: Remove `psc_interface_config` block, set `identity_type = "AGENT_IDENTITY"`, wire `agent_gateway_config` to `var.governance_project_id`'s central gateway, and export `SECRET_NAME`, `GRPC_IPV6_PREFER = "0"`, `GOOGLE_API_USE_MTLS_ENDPOINT = "never"`, and `GOOGLE_GENAI_USE_VERTEXAI = "True"` in `runtime_overrides`.
   * For existing deployed Reasoning Engines, update `agentGatewayConfig` via REST API `PATCH` (`updateMask=spec.deploymentSpec.agentGatewayConfig`).
   * In `agent.yaml`: Add the CAA opt-out (`GOOGLE_API_PREVENT_AGENT_TOKEN_SHARING_FOR_GCP_SERVICES: 'False'`) and trust store variables to `env:`.
6. **`infrastructure/modules/5-governance/modules/1_telemetry_sinks/main.tf` & `3_alert_policies/dashboards.tf`**
   * In `1_telemetry_sinks/main.tf`: Enable BigQuery Log Analytics (`enable_analytics = true`) on the `_Default` Cloud Logging bucket across all monitored projects (including `var.governance_project_id`) so security operators can query Model Armor HTTP 799 rejections and IAP access logs via SQL.
   * In `3_alert_policies/dashboards.tf`: Add two widgets to `agent_golden_signals`:
     1) **Agent Gateway (`AGENT_TO_ANYWHERE`) Request Volume & Latency** (`networkservices.googleapis.com/agent_gateway/request_count`).
     2) **Model Armor Guardrail Rejections (HTTP 799)** (`modelarmor.googleapis.com/inspection_count` by verdict/action).
7. **`apps/agents/a2a-agent/Dockerfile`, `scripts/entrypoint.sh`, & `pyproject.toml` (and `base-adk-agent`)**
   * In `pyproject.toml`: Add `"google-cloud-secret-manager"` to `dependencies`.
   * In `scripts/entrypoint.sh`: Add Secret Manager root CA bundle fetcher (`agw-root-ca-cert`) and register into system trust store (`update-ca-certificates`).
   * In `Dockerfile`: Export `GRPC_DEFAULT_SSL_ROOTS_FILE_PATH=/etc/ssl/certs/ca-certificates.crt`, `REQUESTS_CA_BUNDLE=/etc/ssl/certs/ca-certificates.crt`, and `SSL_CERT_FILE=/etc/ssl/certs/ca-certificates.crt`.
8. **`apps/agents/a2a-agent/agent/__init__.py`**
   * Add `aiohttp.ClientSession(trust_env=True)` monkeypatch so asynchronous HTTP requests route through `http_proxy`/`https_proxy`.
9. **`Makefile`**
   * Replace per-spoke targets (`deploy-agent-gateway-root`, `deploy-agent-gateway-a2a`) with a single centralized target `deploy-agent-gateway-central` pointing to `infrastructure/live/dev/stage-5-governance/agent-gateway`.
   * Add `register-catalog` and verification targets.
10. **`docs/2-workloads-and-catalog/01-ingress-gateways.md`, `02-mcp-tool-servers.md`, `docs/3-agentops-and-lifecycle/03-ai-agents-and-database.md`, `docs/developer-quickstart.md`**
   * Update Phase 3 and Phase 4 platform documentation to explain the Centralized Governance Agent Gateway v2 architecture and central catalog in `var.governance_project_id`.

### 5.3 Files / Resources to DELETE
1. **Per-Spoke PSC Network Attachments (`google_compute_network_attachment`)**
   * Delete `google_compute_network_attachment.psc_attachment` from `infrastructure/modules/4-workloads/agents/a2a-agent/main.tf` and `base-adk-agent/main.tf`.
...
3. **Obsolete Phase 1 Local File Registry (`register_mcp.py` & `mcp_registry.json`)**
   * Delete legacy script `apps/services/register_mcp.py` and local dictionary file `apps/services/mcp_registry.json` (superseded by native GCP Agent Registry in `cloudbuild.yaml`).
   * Delete `null_resource.mcp_registration` from `infrastructure/modules/4-workloads/services/legacy-dms/main.tf`, `income-verification/main.tf`, and `corporate-email/main.tf`.
